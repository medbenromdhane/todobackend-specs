...
describe('Cross Origin Requests', function() {
  ...
  it('should allow all origins', function() {
	var b = {'access-control-allow-origin': '*'};
	// console.log(result);
	// debugger
    return assert(result, "header").to.contain(b);
...
describe('Create Todo Item', function() {
  ...
  it('should receive a location hyperlink', function() {
	// var b = {location: 'http://localhost:8000/todos/'};
	// console.log(result);
	// debugger
    // return assert(result, 'header.location').to.match(/^http:\/\/.+\/todos\/[\d]+$/);
    return assert(result, 'header.location').to.deep.match(/^http:\/\/.+\/todos\/[\d]+$/);
  });
  ...
  it('should create the item', function() {
	...
	return assert(result, 'body.title').to.deep.match(/^Walk the dog$/);
  });
